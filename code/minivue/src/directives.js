    ...
    update (value) {
      ...
      for (let key in value) {
        ...
        if (this.valueKey) {
          ...
          re3 = /{{\s*(.*)\.(.*)\s*}}/g
          // 可以匹配出
          // ["{{ item.rank }}", "{{ item.title }}", "{{ item.desc }}"]
          let _matchArr = html.match(re3)
          if (_matchArr && _matchArr.length > 0) {
            let self = this
            _matchArr.forEach(match => {
              // match => "{{ item.rank }}"
              let matchedStr = match.match(/{{\s*(.*)\s*}}/)[1].trim()
              // matchedStr => "item.rank" or "item.rank.rank"
              let matchedArr = matchedStr.split('.')
              if (matchedArr[0] === `${self.valueKey}`) {
                let len = 1
                let _value = value[key]
                while (len < matchedArr.length) {
                  // console.log(_value)
                  _value = _value[matchedArr[len]]
                  len++
                }
                // 此时 _value 是最终的值
                // console.log(_value)
                html = html.replace(match, _value)
              }
            })
          }
        ...
        // 问题出在这里 item in items
        // 此处只能匹配出 item
        // 不能匹配到 item.rank item.title item.desc


        if (this.indexKey) {
          re2 = new RegExp(`{{\\s*${this.indexKey}\\s*}}`, 'g')

          // 还需要将作为参数传递的 index 替换掉 @click="remove(index)" => @click="remove(1)"
          re4 = new RegExp(`\(${this.indexKey}\)`, 'g')
          // debugger
          html = html.replace(re2, key)
          html = html.replace(re4, key)
        }
