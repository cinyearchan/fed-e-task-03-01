...
function parseMethodExpression (exp) {
  exp = exp.trim()
  // remove(0)
  return makeMethodGetterFn(exp)
}

function makeMethodGetterFn (exp) {
  // 将 remove(0) 解析成 方法名 remove 参数 0
  let reg = /(.*)\((.*)\)/
  let match = exp.match(reg) // ["remove"]
  let fnName = match[1]
  let args = match[2]
  return new Function('vm', 'return vm.' + fnName + '.bind(vm, ' + args + ')')
}
...
export function parsePath (path) {
  if (path.includes('(') && path.includes(')')) {
    return parseMethodExpression(path)
  }
  if (bailRE.test(path)) {
    return
  }
